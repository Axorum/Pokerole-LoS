...
export class PokeroleActorSheet extends ActorSheet {
  ...
  async getData() {
    ...
    // TP support.
    context.genre = {None: "None", Male: "Male", Female: "Female"};
    ...
    context.lockdir = {lock: "Lock", unlock: "Unlock"};
  ...
  async _prepareItems(context) {
    ...
    for (let i of context.items) {
      ...
      if (i.type === 'item') {
        ...
        // TP Support 
        let categorylist = (i.flags['item-piles']?.item.customCategory ?? 'Misc Item');

        if (!customitemdf[categorylist]) {
          customitemdf[categorylist] = {
            catname: categorylist, 
            customitemlist: []
          }
        };
    ...
    for (const categorylist of Object.keys(game.itempiles.API.getItemCategories())) {
      if (customitemdf[categorylist]) {
        customitemdfordered[categorylist] = customitemdf[categorylist];
      };
    }
  ...
  async _showSettings() {
    const { baseHp, willbonus, ishuman, customInitiativeMod, hasThirdType, recommendedRank, source } = this.actor.system;
